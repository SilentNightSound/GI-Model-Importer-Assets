...
def main():
    ...
    parser.add_argument("-r", "--recent", action='store_true', help="(Deprecated, default behaviour) Use the most recent framedump folder instead of specifying")
    ...
    for i in range(len(model_data)):
        ...
        output_results(frame_dump_folder, character, args.component_names, model_data, vb_merged, position_vb, i, texture_only, base_classification)
...
def collect_relevant_ids(frame_dump_folder, draw_vb_hashes):
    relevant_ids = []
    first_vss = []
    frame_dump_files = os.listdir(frame_dump_folder)
    ...
    for i in range(len(relevant_ids)):
        print(f"\tComponent{i+1}: {relevant_ids[i]}")
...
def collect_model_data(frame_dump_folder, relevant_ids, force_ids):
    ...
    for id_group in relevant_ids:
        ...
        for current_id in id_group:
            ...
            texcoord_vb_candidate = []
            if texture_only:
                model_group[first_index] = ["_"]
            else:
                model_group[first_index] = [draw_ib]
                ...
                if len(texcoord_vb_candidate) == 1:
                    ...
                    texcoord_size = os.stat(os.path.join(frame_dump_folder, texcoord_vb_candidate)).st_size
                ...
                position_size = os.stat(os.path.join(frame_dump_folder, position_vb_candidate)).st_size
                if position_size > position_largest_size:
                    position_largest_size = position_size
...
def collect_buffer_data(frame_dump_folder, filename, filters):
    group_size = len(filters)
    result = []
    with open(os.path.join(frame_dump_folder, filename), "r") as f:
        headers, data = f.read().split("vertex-data:\n")
        element_format = parse_buffer_headers(headers, filters)
...
def output_results(frame_dump_folder, character, component_names, model_data, vb_merged, position_vb, current_part, texture_only, object_classifications):
